...
- `memos/vault/policy/` is doctrine. It is slow-changing, non-negotiable guidance that
  overrides memos.
...
- Intent is not a tracker; execution details belong in companion notes or
  README files.
- Doctrine lives in `memos/vault/policy/` and captures long-lived defaults.
